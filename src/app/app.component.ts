...
@Component({
  ...
  entryComponents: [CheckoutPage, CartPage, ProductsPage, CategoriesPage]
})
export class MyApp {
  ...
  postOauthRedirect(customerJourney, navigator){

    switch(customerJourney.split('.')[0]){
      default:
      case 'something-happened':
        break;
      case 'dbs-rewards-2fa':
        navigator.push(CheckoutPage, { provider : 'dbs-2fa' });
        break;
      case 'dbs-rewards': 
        navigator.push(CheckoutPage, { provider : 'dbs' }); 
        break;
      case 'braintree': 
        navigator.push(CheckoutPage, { provider : 'braintree' });
        break;
    }
  ...
  initializeApp() {
    ...
    console.log('App Loaded into View Cycle');

    let userCheckOutJourney = this.configuration.get('user-checkout-journey');

    if(userCheckOutJourney && 
       userCheckOutJourney.indexOf('auth-pending') !== -1){

      this.configuration.set('user-checkout-journey', userCheckOutJourney.replace('auth-pending.', ''));
      this.postOauthRedirect(userCheckOutJourney.replace('auth-pending.', ''), this.nav);
    }
